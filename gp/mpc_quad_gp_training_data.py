'''Runs MPC example simulations to generate training data for FMPC+SOCP Gaussian process'''
...
# modified to work with mpc training data generation, similar to mingxuans scripts for benchmarking
def run(gui=True, n_episodes=1, n_steps=None, save_data=False, ALGO = 'fmpc', ADDITIONAL = ''):
    ...
    assert os.path.exists(f'./config_overrides/{SYS}/{ALGO}_{SYS}_{TASK}.yaml'), f'./config_overrides/{SYS}/{ALGO}_{SYS}_{TASK}.yaml does not exist'

    sys.argv[1:] = ['--algo', ALGO,
                    '--task', agent,
                    '--overrides',
                        f'./config_overrides/{SYS}/{SYS}_{TASK}{ADDITIONAL}.yaml',
                        f'./config_overrides/{SYS}/{ALGO}_{SYS}_{TASK}.yaml',
                    '--seed', repr(seed),
                    '--use_gpu', 'True',
    ...
    if save_data:
        ...
        path_dir = os.path.dirname('./temp-gp-training-data/')
        os.makedirs(path_dir, exist_ok=True)
        with open(f'./temp-gp-training-data/{config.algo}_data_{config.task}_{config.task_config.task}.pkl', 'wb') as file:
