...
def run(gui=True, n_episodes=1, n_steps=None, save_data=False, algo='', yaml_base='', yaml_ctrl=''):
    ...
    assert os.path.exists(yaml_base), f'{yaml_base} does not exist'
    assert os.path.exists(yaml_ctrl), f'.{yaml_ctrl} does not exist'

    sys.argv[1:] = ['--algo', algo,
                    '--task', agent,
                    '--overrides', yaml_base, yaml_ctrl]
